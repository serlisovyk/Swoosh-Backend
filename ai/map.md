...
| `email` | Resend + `@react-email/render`; each template is a folder under `templates/<name>/` (currently `reset-password`) with `template.tsx` (JSX), `styles.ts` (inline-style objects), and a barrel `index.ts` re-exporting the component — callers import `./templates/<name>`, never the file paths inside it; subjects in `email.constants.ts`; a send failure (provider error or rejected promise) is logged with recipient/subject/provider detail and rethrown as `InternalServerErrorException(EMAIL_SEND_FAILED_ERROR)` — callers never see the provider's own error |
...
| `config/collect-constraint-messages.utils.ts` | `collectConstraintMessages` — recursive `class-validator` `ValidationError[]` → flat message list |
| `config/env/*.env.ts` | one file per domain class: `app.env.ts` (`AppConfig`), `cors.env.ts` (`CorsConfig` — `CORS_DOMAINS` always required, no environment-conditional default), `jwt.env.ts` (`JwtConfig`), `captcha.env.ts` (`CaptchaConfig`), `swagger.env.ts` (`SwaggerConfig`) + `parse-swagger-enabled.utils.ts` (`SWAGGER_ENABLED` coercion, reuses `toBooleanQueryParam`), `mongo.env.ts` (`MongoConfig`), `email.env.ts` (`EmailConfig`), `throttler.env.ts` (`ThrottlerConfig`) — every domain validates independently, no field depends on another domain's value |
...
| `utils/env.utils.ts` | `getEnv`/`getEnvOrThrow` (wrap `ConfigService.get`/`getOrThrow` with `{ infer: true }`, typed off `AppEnv`'s nested shape) / `isDev` / `isProd` / `parseCorsDomainsConfigValue` |
