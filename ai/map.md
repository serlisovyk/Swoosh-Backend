...
|---|---|
| `captcha` | Cloudflare Turnstile wrapper: `captcha.module.ts` (`TurnstileModule.forRootAsync`), `captcha.config.ts` (`getCaptchaConfig`), `captcha.utils.ts` (`createCaptchaException`, `getTokenFromResponse`), `captcha.constants.ts` (`CAPTCHA_TOKEN_HEADER` = `cf-turnstile-token`); the `@Captcha()` decorator — applied on auth and password-reset endpoints |
