...
| `favorites` | favorites list; no schema of its own — depends on `UsersModule`/`ProductsModule` and calls `UsersService`/`ProductsService`, never injects their models directly | `favorites.service.ts`, `favorites.utils.ts` (pagination/meta), `dto/find-all-favorites.dto.ts` |
