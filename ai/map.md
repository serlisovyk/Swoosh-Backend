...
| `favorites` | favorites list; no schema of its own — depends on `UserModule`/`ProductsModule` and calls `UserService`/`ProductsService`, never injects their models directly. Exports a barrel (`index.ts`: `FavoritesService`, `FavoritesModule`, the favorites-limit constants and Swagger property docs `auth`'s DTOs/`user.swagger.ts` need) — other modules import from `@modules/favorites`, never the file paths below | `favorites.service.ts`, `favorites.utils.ts` (pagination/meta), `dto/find-all-favorites.dto.ts` |
