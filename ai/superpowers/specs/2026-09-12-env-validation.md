...
- Add an `AppEnv` class-validator class (`src/shared/config/env.config.ts`) covering every key in `.env.sample`, validated via `plainToInstance` + `validateSync` inside `validateEnv()`, which is passed to `ConfigModule.forRoot` as `validate`. `validateSync` collects every failing property in one pass (not just the first), so a failing env throws one `Error` at bootstrap listing every invalid/missing key at once. `class-validator`/`class-transformer` are already dependencies (every DTO uses them) — no new dependency, unlike an earlier version of this change that added `zod` (see the decision record for why that was reverted per author feedback).
- `validate`'s return value is what `ConfigService` serves for `.get()`. `@Transform` decorators on the class do the necessary coercion (`PORT`/`THROTTLE_TTL`/`THROTTLE_LIMIT` → `number` via `@Type(() => Number)`, `CORS_DOMAINS` → `string[]`, `SWAGGER_ENABLED` → `boolean`), so the values coming out of `ConfigService` are already the right runtime type — no cast needed at the read site.
- Type all `ConfigService` injection points as `ConfigService<AppEnv, true>` (`AppEnv` is the class itself, used only as a type everywhere outside `env.config.ts`; `true` = disallow `.get()` falling back to `undefined` for keys the schema declares). Reads become `configService.get('KEY', { infer: true })` — no explicit `<T>` argument, because the type comes from the schema, not from an assertion at the call site. This is the direct fix for defect 1: the generic isn't removed everywhere (`getOrThrow` stays where "crash if a genuinely optional-shaped value turns out missing" is still the right runtime behavior, e.g. `SWAGGER_USER`/`SWAGGER_PASSWORD` outside dev), but it stops being a type-level lie because the base type is now schema-derived, not asserted.
...
- `NODE_ENV=production` → `CORS_DOMAINS` required, non-empty (`@ValidateIf((env) => env.NODE_ENV === NODE_ENV.PRODUCTION)` gating `@IsArray()`, not just `@IsOptional()`).
...
The existing `parseCorsDomainsConfigValue` (comma-split, trim, drop empties) is reused inside the property's `@Transform` rather than reimplemented — it now always runs against a guaranteed-present string (either the real value or the dev default) in development, so `AppEnv['CORS_DOMAINS']` only ever surfaces as `undefined` (failing `@IsArray`) in the one case that should fail: production with nothing set.
...
`ms`'s `StringValue` is a branded template-literal type no runtime validator here expresses as a checked shape without an elaborate, easy-to-get-wrong regex (out of scope per the issue's "no business-format validation beyond type/requiredness"). The schema validates these as non-empty strings (guaranteed present, matching every other secret/required key); the two call sites in `auth.service.ts` keep a single explicit `as StringValue` cast, documented with a one-line comment. This is a narrower, honest cast at the one real type-system boundary, replacing the previous `getOrThrow<StringValue>()` call that asserted the same thing with no runtime backing at all.

### `@IsUrl` needs `require_tld: false` (found while implementing)

`class-validator`'s `@IsUrl()` rejects `http://localhost:*` by default (`validator.js`'s `isURL` requires a TLD unless told otherwise) — both `CLIENT_URL` and `SERVER_URL` point at `localhost` in dev. `@IsUrl({ require_tld: false })` on both. This is a real gap the zod-based first pass of this change didn't have (`z.url()` accepts `localhost` with no option needed) — worth calling out since it would have broken every local dev boot.

### `NODE_ENV` as-const (small, necessary prerequisite)

`src/shared/constants/env.constants.ts`'s `NODE_ENV` object is declared without `as const`, so `@IsEnum(NODE_ENV)` and the `AppEnv['NODE_ENV']` field type need real string literals, not the widened `string` type the object currently produces. Adding `as const` is required to build the schema at all — this is also item 2 of MY-52 (still Todo). Doing this one-line fix here (and noting it in the decision record) avoids MY-52 redoing it, per the issue's own coordination note. `isProd` (also named in MY-52) is added next to `isDev` for the same reason — the schema/CORS logic above needs a prod check.
...
- No new validation dependency — `zod` was tried first, then dropped for `class-validator`/`class-transformer` (already dependencies) per author feedback; see [decisions/env-validated-at-boot](../../decisions/2026-09-12-env-validated-at-boot.md).
