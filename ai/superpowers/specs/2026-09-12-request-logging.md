...
- `request-logging.middleware.ts` — `requestLoggingMiddleware(request, response, next)`:
  ...
  - Registers a `response.on('finish', ...)` listener that computes duration, builds the `RequestLogPayload`, skips emission when `request.path` is one of `SKIP_LOGGING_PATHS` and the response was successful (< 400), and otherwise logs via a module-level `Logger`: `error` for ≥500, `warn` for ≥400, `debug` otherwise.
- `index.ts` — barrel: `requestLoggingMiddleware`, `REQUEST_ID_HEADER`.
...
- `src/main.ts`: `NestFactory.create(AppModule, { logger: [...] })` — the active Nest log levels exclude `'debug'`/`'verbose'` outside development (checked via raw `process.env.NODE_ENV`, since `ConfigService` isn't available until after `create()` resolves). This is what actually keeps successful-request logging quiet in production (the per-request `debug` call becomes a no-op there), not just the path-skip above.
