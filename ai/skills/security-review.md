...
- **CORS & config** — credentialed CORS restricted to expected origins; `CORS_DOMAINS` is required at boot in production (schema-enforced, see [decisions/env-validated-at-boot](../decisions/2026-09-12-env-validated-at-boot.md)) — flag any change that makes it optional again as a silent-`*`-origin regression. Secrets are read via `configService.get('NAME', { infer: true })` at the point of use, never hard-coded, declared in both `src/shared/config/env.config.ts` and `.env.sample`, and never logged. A `getOrThrow<T>`/`get<T>` with an explicit type argument is a regression — the type must come from the schema, not an assertion.
