...
- **Embedded subdocuments**: embed the generated schema (`type: [ProductColorSchema]`) for arrays; embed the class (`type: Address, default: {}`) for a single nested object. Either way, the embedded class itself is `@Schema({ _id: false, versionKey: false })`, never `{ timestamps: true }` — `timestamps` is for top-level collections; on an embedded object it generates an unused `_id`/`createdAt`/`updatedAt` per subdocument. `ProductColor` (array) and `Address` (single nested, `user-address.model.ts`) are both reference examples.
