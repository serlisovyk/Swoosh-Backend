...
## Application Bootstrap (`src/shared/config/app.config.ts` + `src/main.ts` / `src/serverless.ts`)

All app-level wiring lives in one function, `setupApp(app)` (`src/shared/config/app.config.ts`), not inline in `main.ts`. It is called by both entry points — `src/main.ts` (`app.listen`, Docker/local dev) and `src/serverless.ts` (`app.init`, Vercel — see [decisions/vercel-serverless-entry](../decisions/2026-09-12-vercel-serverless-entry.md)) — so the two never drift apart. Do not duplicate any of this wiring at a call site; add to `setupApp` instead.
...
- `bootstrap()` (in `main.ts`) is called as `bootstrap().catch((error) => { logger.error(...); process.exit(1) })`, never bare `void bootstrap()`. A startup failure (bad env, unreachable Mongo, anything thrown during wiring) now always logs and exits 1 instead of surfacing as a silent unhandled rejection. `serverless.ts`'s handler has no equivalent top-level catch — a startup failure there surfaces as a rejected request instead, which is the correct behavior for a per-invocation serverless function.
...
- `PORT` is a required env var (validated at boot by the `AppEnv` schema), read in `main.ts` with `getEnv(configService, 'app.PORT')` off the `ConfigService` `setupApp` returns — `serverless.ts` never listens on a port, so it has no use for it.
- Keep global wiring in `setupApp`; do not scatter global config into feature modules, and do not re-inline any of it directly in `main.ts` or `serverless.ts`.
