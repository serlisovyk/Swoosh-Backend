...
- The refresh token's lifetime has one source: `JWT_REFRESH_TOKEN_EXPIRES_IN` (`ms` format, e.g. `"1d"`), schema-required at boot (`src/shared/config/env.config.ts`, see [decisions/env-validated-at-boot](../decisions/2026-09-12-env-validated-at-boot.md)). It signs the JWT and derives the cookie's `expires` date — do not add a second env var for the cookie's lifetime.
- `JWT_ACCESS_TOKEN_EXPIRES_IN` is also schema-required at boot. Never let either token's `expiresIn` fall through to `undefined` — that issues a JWT with no expiry.
...
- `POST /auth/logout` clears the refresh cookie; it does not revoke already issued stateless refresh tokens before expiry. `AuthService` has no `logout` method — the controller only clears the cookie.
- The access-token payload carries only `id`. Role comes from the DB-loaded user on every request (`JwtStrategy.validate` → `UsersService.getById`), never from the token — see [decisions/access-token-role-stays-server-side](../decisions/2026-09-12-access-token-role-stays-server-side.md).
- Cookie-setting (`setRefreshTokenCookie`/`clearRefreshTokenCookie`) lives in `src/modules/auth/auth.cookies.ts`, called from `AuthController` — `AuthService` returns tokens only and does not import `express`.
