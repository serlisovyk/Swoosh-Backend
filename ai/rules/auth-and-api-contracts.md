...
- The access-token payload carries only `id`. Role comes from the DB-loaded user on every request (`JwtStrategy.validate` → `UserService.getById`), never from the token — see [decisions/access-token-role-stays-server-side](../decisions/2026-09-12-access-token-role-stays-server-side.md).
