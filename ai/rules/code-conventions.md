...
- Swagger `description`/`summary` text is written in Russian; `ApiTags` category names, code, comments, and identifiers stay in English. Every module's Swagger is translated, including the shared helpers in `@common/errors/errors.swagger.ts` and `system.swagger.ts`. Don't half-translate a module: when you touch a module's Swagger for another reason, translate the rest of that file in the same change.
- Actual API response text (not just docs) is Russian too — the error `message` fields returned by `AllExceptionsFilter` (`INTERNAL_ERROR_MESSAGE`, `VALIDATION_ERROR_MESSAGE` in `src/common/errors/errors.constants.ts`), and the default `UnauthorizedException`/`ForbiddenException` messages a guard would otherwise let Nest fill in with its English defaults — `JwtAuthGuard`/`RolesGuard` (`src/modules/auth/guards`) throw explicit Russian-message exceptions instead of relying on Nest's built-in wording.
...
- Inline one-off validation messages, Swagger descriptions, and examples when they are only used locally and extraction hurts readability — see `src/modules/auth/auth.constants.ts` (and `password-reset/password-reset.constants.ts`), trimmed to only the values reused across files after everything single-use was inlined.
- Remove stale constants after deleting features.
- Even module-private constants (a status→code lookup map, a threshold used only inside one filter/service) belong in `<feature>.constants.ts`, not declared at the top of the class file that uses them. Keeps the class file to behavior, keeps constants greppable in one place — see `src/common/errors/error-codes.constants.ts` (`STATUS_TO_ERROR_CODE`) vs `all-exceptions.filter.ts`.
- Do not alias a library's own enum members (e.g. `HttpStatus.BAD_REQUEST`) behind a project constant just to get a plain-`number` type. Reference the enum member directly; if a comparison then trips `no-unsafe-enum-comparison`, type the value as the enum (`HttpStatus`, not `number`) where it's first produced — a function's return type, an interface field, a local's declared type — instead of an `eslint-disable-next-line` or an `as` cast. See [decisions/no-number-typed-http-status-aliases](../decisions/2026-09-12-no-number-typed-http-status-aliases.md).
- A constant lives at the lowest level every one of its consumers can import — never copy the same literal into two files "because it's easier". If a `common/*` package and a feature module both need the same value, it belongs in `shared/constants`, and each side imports (or re-exports) it — see `API_PREFIX`/`SWAGGER_DOCS_PATH` (`shared/constants/api.constants.ts`, read by `main.ts` and `shared/config/swagger.config.ts`) and `REFRESH_TOKEN_COOKIE_NAME` (`shared/constants/cookie.constants.ts`, read by `modules/auth` and `shared/constants/swagger.constants.ts`'s `SWAGGER_REFRESH_TOKEN_AUTH_NAME`). Two independent literals that are supposed to stay equal will eventually drift.
