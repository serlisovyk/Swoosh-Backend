...
export function ProductsFindAllDocs() {
  return applyDecorators(
    ApiOperation({
      ...
      description: `Возвращает отфильтрованный список товаров с общим количеством. Поддерживает фильтры каталога и полнотекстовый поиск. Если переданы ids (не более ${MAX_PRODUCTS_LIMIT} id за раз, так же как excludeIds), порядок ответа сохраняет порядок из запроса, а sort игнорируется — но page и limit применяются к результату как обычно.`,
      security: [],
    }),
    ApiOkResponse({
